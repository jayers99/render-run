...
| Aspect | CLI Wrappers (Gemini CLI, `gpt-image`, etc.) | Direct API / SDK Calls |
| --- | --- | --- |
...
**Current leaning:** start with direct API calls for portability + control, but document CLI installation as a stopgap (especially while API keys are being provisioned). Revisit once we confirm Gemini image support path.
